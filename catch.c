 ...
 * This code is inspired from : https://gist.github.com/bellbind/6813905
 */
...
int j=0;
while(j<i){
  camera_t* camera = camera_open("/dev/video0", 352, 288);/*Initializing the device*/
  camera_init(camera);
  camera_start(camera);/*Open the webcam*/
  ...
  unsigned char* rgb = 
    yuyv2rgb(camera->head.start, camera->width, camera->height);/*Convert from YUV space to RGB*/
           /*name of file*/
	sprintf(buf,"result%d.jpg",j);
  FILE* out = fopen(buf, "w");
	

  jpeg(out, rgb, camera->width, camera->height, 100);/*Write data to file*/
  fclose(out);
  free(rgb);

 /*Stoping and closing webcam*/ 
  camera_stop(camera);
